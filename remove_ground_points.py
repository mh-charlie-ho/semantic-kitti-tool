...
def remove_points(point, label):
    global ground_point_label
    checkedList = np.full(len(label), False, dtype=bool)
    for n in ground_point_label:
        checkedList = checkedList | (label == n)
...
for i in range(len(bin_paths)):
    ...
    label = lf.load_label(label_path)
    new_pt, new_label = remove_points(pt, label)
